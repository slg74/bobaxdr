...
async def ingest_event(
    ...
    if data.get("type") == "aws_scan":
        background_tasks.add_task(
            _process_aws_scan, data.get("instances", []), data.get("region", "us-east-1")
        )
        ...
        for etype in ("process_snapshot", "network_connections"):
            ...
            if etype == "process_snapshot":
                ...
                external = [
                    ...
                    if c.get("raddr")
                    and not any(
                        c["raddr"].startswith(p) for p in ("127.", "192.168.", "10.", "::1", "169.")
                    )
                ]
                result.append(
                    {
...
async def dashboard_stats(db: Session = Depends(get_db), _=Depends(auth)):
    ...
    alerts_24h = (
        db.query(Alert).filter(Alert.timestamp > since_24h, Alert.acknowledged == False).count()
    )
    critical_24h = (
        db.query(Alert)
        .filter(
...
async def list_cloud_instances(db: Session = Depends(get_db), _=Depends(auth)):
    return [
        i.to_dict()
        for i in db.query(CloudInstance).order_by(CloudInstance.region, CloudInstance.name).all()
    ]
