...
def _pod_issues(pod) -> list[dict]:
    ...
    if spec.host_network and ns not in host_ns_exempt:
        issues.append(
            {"severity": "high", "description": "Pod using hostNetwork — shares host network stack"}
        )
    if spec.host_pid and ns not in host_ns_exempt:
        issues.append(
            {"severity": "high", "description": "Pod using hostPID — can see all host processes"}
        )
    ...
    for vol in spec.volumes or []:
        if vol.host_path:
            ...
            if ns in SYSTEM_NS and any(path.startswith(s) for s in SYSTEM_HOSTPATH_OK):
                continue
            issues.append({"severity": "high", "description": f"Sensitive hostPath volume: {path}"})
...
def poll(session: requests.Session):
    try:
        ...
        logger.info(
            f"K8s: {len(pods)} pods, {crash} crashloop, "
