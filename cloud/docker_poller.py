...
INFRA_IMAGES = {
    "kindest/node",
    "docker/desktop-cloud-provider-kind",
    "docker/desktop-containerd-registry-mirror",
    "envoyproxy/envoy",
}

SENSITIVE_PATHS = {"/etc", "/var/run/docker.sock", "/proc", "/sys", "/root", "/run/containerd"}
...
def _check_container(c) -> list[dict]:
    ...
    if hc.get("Privileged"):
        issues.append(
            {"severity": "critical", "description": "Container running with --privileged"}
        )

    if hc.get("NetworkMode") == "host":
        issues.append(
            {"severity": "high", "description": "Container using host network (--net=host)"}
        )
    ...
    for mount in attrs.get("Mounts", []):
        ...
        if any(src.startswith(p) for p in SENSITIVE_PATHS):
            issues.append(
                {"severity": "high", "description": f"Sensitive host path mounted: {src}"}
            )
